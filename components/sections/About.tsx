    ...
    <section id="about" className="relative py-16 overflow-hidden"
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6">
        <motion.div className="text-center mb-10"
          ...
          <h2 className="font-heading font-bold leading-[1.05] mb-3"
            ...
            <br />
            <span className="font-light" style={{ fontSize: '0.6em', color: 'var(--text-muted)' }}>
              &amp; Full Stack Engineer
            </span>
          </h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr,auto] gap-8 items-start mb-10">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
            ...
            <p className="leading-relaxed text-sm mb-5" style={{ color: 'var(--text-secondary)' }}>
              <strong style={{ color: 'var(--text-primary)' }}>Ajay Patil</strong> — BCA Final Year (2025-26) from Palus, Maharashtra.
              Specializing in <strong style={{ color: '#4ade80' }}>Java backend development</strong> with Spring Boot,
              Spring Security, JPA, and Hibernate. Built <strong style={{ color: 'var(--text-primary)' }}>CrowdSpark-X</strong> (PHP → Spring Boot microservices).
          ...
          <motion.div className="grid grid-cols-2 gap-2.5"
            ...
            {quickInfo.map((item, i) => (
              <div key={i} className="flex flex-col gap-1 px-3 py-2.5 rounded-lg text-center"
        ...
        <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-3"
          ...
          {skills.map((s, i) => (
            <motion.div key={i} className="rounded-xl p-4 text-center"
              style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}
              whileHover={{ y: -3, borderColor: 'var(--border-strong)', transition: { duration: 0.2 } }}>
